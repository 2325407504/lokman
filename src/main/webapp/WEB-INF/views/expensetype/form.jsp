...
<ul class="nav nav-tabs">
	...
	<c:choose>
		<c:when test="${ !empty expensetypeAttribute.id }">
			<li class="active"><a href="${expensetypeEdit}"><spring:message code="Entry No"></spring:message>: ${expensetypeAttribute.id}</a></li>
		</c:when>
		<c:otherwise>
			<li class="active"><a href="${expensetypeNew}"><spring:message code="New Entry"></spring:message></a></li>
		</c:otherwise>
	</c:choose>
</ul>
...
<form:form modelAttribute="expensetypeAttribute" action="${expensetypeSave}" method="post">
	...
	<fieldset>
		...
		<div class="form-actions">
			...
			<a class="btn btn-danger" href="javascript:$('#form-${expensetypeAttribute.id}').submit();"><spring:message code="Delete"></spring:message></a>
			</c:if>
