package com.aripd.project.lokman.report.forwarding;
...
public class FillManager {
	...
	public static void fillReport(HSSFSheet worksheet, int startRowIndex,
		...
		for (int i = startRowIndex; i + startRowIndex - 2 < datasource.size() + 2; i++) {
			...
			cell4.setCellValue(datasource.get(i - 2).getStartingAt().toString());
			cell4.setCellStyle(bodyCellStyle);

			HSSFCell cell5 = row.createCell(startColIndex + 4);
			cell5.setCellValue(datasource.get(i - 2).getEndingAt().toString());
			...
			cell8.setCellValue(datasource.get(i - 2).getShippingCost());
