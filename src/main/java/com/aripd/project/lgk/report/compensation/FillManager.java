...
public class FillManager {
    ...
    public static void fillReport(HSSFSheet worksheet, int startRowIndex, int startColIndex, List<Compensation> datasource) {
        ...
        for (int i = startRowIndex; i + startRowIndex - 2 < datasource.size() + 2; i++) {
            ...
            cell0.setCellValue(datasource.get(i - 2).getId());
            cell0.setCellStyle(bodyCellStyle);

            HSSFCell cell1 = row.createCell(startColIndex + 1);
            cell1.setCellValue(formatter.print(datasource.get(i - 2).getProduction().getShiftdate()));
            cell1.setCellStyle(bodyCellStyle);

            HSSFCell cell2 = row.createCell(startColIndex + 2);
            cell2.setCellValue(datasource.get(i - 2).getElectricmeter().getName());
            cell2.setCellStyle(bodyCellStyle);

            HSSFCell cell3 = row.createCell(startColIndex + 3);
            cell3.setCellValue(datasource.get(i - 2).getVal());
